use twilio_rust::models::models::DeleteMediaMessage;
use twilio_rust::Twilio;

pub async fn delete_media_message(account_sid: String, auth_token: String) {
    ...
    if let Ok(_twilio) = _result {
        ...
        let _result = DeleteMediaMessage::new(message_sid, _sid);

        if let Ok(_message) = _result {
            let _output = _twilio.delete_media_message(_message);
